...
async function dashboardData(config, accessToken) {
  try {
    const requests = await Promise.all([
      ...
      supabaseRequest(config, '/rest/v1/integration_connections?select=id,provider,display_name,status,last_synced_at&order=provider', { accessToken }),
...
function googleMediaPage(google) {
  ...
  if (google.state === 'disconnected' || google.state === 'expired') return `<section class="page-heading"><p class="eyebrow">Google Drive</p><h1>Media</h1><p>Browse the folders and Google Docs available to your authorized Google account, directly inside Console.</p></section><section class="drive-connect"><span class="drive-logo">G</span><div><h2>${google.state === 'expired' ? 'Your Google session has expired' : 'Connect Google Drive'}</h2><p>Start a short-lived, read-only session to browse folders and open Google Docs. Console will not create, edit, move, or delete Drive content.</p>${connect}</div></section>`;
  if (google.state === 'error') return `<section class="page-heading"><p class="eyebrow">Google Drive</p><h1>Media</h1></section><section class="drive-connect error"><span class="drive-logo">!</span><div><h2>Drive needs attention</h2><p>${escapeHtml(google.message || 'Google Drive could not load.')}</p>${connect}</div></section>`;
  if (google.document) {
...
function consolePage(user, dashboard, notice = '', requestToken = '', selectedView = 'overview', googleSession = { state: 'disconnected' }) {
  const views = ['overview', 'productions', 'media', 'integrations', 'storage', 'activity', 'settings'];
  ...
  const workspaceOptions = operatorWorkspaces.map((organization) => `<option value="${escapeHtml(organization.id)}">${escapeHtml(organization.name)}</option>`).join('');
  const nav = [
    ['overview', 'Overview', '◌'], ['productions', 'Productions', '◇'], ['media', 'Media', '▤'], ['integrations', 'Integrations', '⌁'],
  ...
  const pages = { overview, productions: productionView, media: googleMediaPage(googleSession), integrations: integrationView, storage: storageView, activity: activityView, settings: settingsView };
...
exports.handler = async (event) => {
  ...
  if (form) {
    ...
    if (result.ok) {
      ...
      return responsePage(consolePage(user, dashboard, message, requestToken, selectedView, googleDrive));
    }
    return responsePage(consolePage(user, dashboard, result.message, requestToken, selectedView, googleDrive));
  }

  return responsePage(consolePage(user, dashboard, '', requestToken, selectedView, googleDrive));
};
