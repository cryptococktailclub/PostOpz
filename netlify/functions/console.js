...
function slackPage(slack, requestToken) {
  ...
  return `${intro}${control}<div class="drive-toolbar slack-toolbar">${back}${refresh}<a href="/console/slack/connect">Manage channels</a></div>${composer}<section class="slack-panel"><div class="panel-heading"><div><p class="eyebrow">${slack.threadTs ? 'Conversation thread' : 'Live channel view'}</p><h2>#${escapeHtml(slack.selected.name)}</h2><p class="quiet">Latest messages are shown here. Scroll for earlier indexed conversation.</p></div><span class="status-pill healthy"><i></i>${slack.canPost ? 'Read & write' : 'Read only'}</span></div><div class="main-slack-feed">${rows}</div></section>${alerts}`;
}
...
function productionWorkspaceView(dashboard, requestToken, productionId, user) {
  ...
  const activityRows = activity.length
    ? `<div class="production-activity-feed"><div class="event-list production-events">${activity.map((item) => `<article class="event"><span class="event-dot ${escapeHtml(item.severity || 'info')}"></span><div><strong>${escapeHtml(item.title)}</strong><p>${escapeHtml(item.detail || 'No additional details')}</p></div><time>${escapeHtml(shortDate(item.occurred_at))}</time></article>`).join('')}</div></div>`
  ...
  const productionSlackTranscript = linkedSlackChannelIds.size
    ? `<section class="slack-panel production-slack-thread"><div class="panel-heading"><div><p class="eyebrow">Production Slack</p><h2>Channel conversation</h2><p class="quiet">Latest messages are shown here. Scroll for earlier indexed conversation.</p></div><span class="status-pill healthy"><i></i> Scoped</span></div><div class="production-slack-feed" data-production-slack-feed data-production-id="${escapeHtml(production.id)}">${productionSlackRows}</div></section>`
    : '';
...
function consolePage(user, dashboard, notice = '', requestToken = '', selectedView = 'overview', googleSession = { state: 'disconnected' }, slackSession = { state: 'unregistered' }, selectedProductionId = '', productionOnly = false) {
  ...
  const pairedPremiereWorkstations = (dashboard.premiereAgents || []).length;
  const operatorWorkspaces = operatorOrganizations(dashboard);
  const workspaceOptions = operatorWorkspaces.map((organization) => `<option value="${escapeHtml(organization.id)}">${escapeHtml(organization.name)}</option>`).join('');
  const navLink = ([id, label, icon]) => `<a class="nav-item ${id === view ? 'active' : ''}" href="/console?view=${id}"><span>${icon}</span>${label}</a>`;
  const primaryNav = [
    ['overview', 'Overview', '◌'], ['productions', 'Productions', '◇'], ['media', 'Workspace Files', '▤'], ['slack', 'Slack', 'S'], ['activity', 'Activity', '↗']
  ].map(navLink).join('');
  const systemNav = [
    ['integrations', 'Integrations', '⌁'], ['storage', 'Storage', '▣'], ['settings', 'Settings', '⚙']
  ].map(navLink).join('');
  const globalNav = `<div class="nav-group">${primaryNav}</div><p class="nav-label">System</p><div class="nav-group system-nav">${systemNav}</div>`;
  ...
  const overview = `<section class="hero console-welcome"><div><p class="eyebrow">${escapeHtml(dashboard.organizations.map((organization) => organization.name).join(' · ') || 'PostOpz workspace')}</p><h1>Your production desk.</h1><p class="hero-copy">A focused view of the work, people, and source activity that needs attention today.</p></div><aside class="hero-status"><span class="live-dot"></span><div><b>Console is online</b><p>${healthy} connected source${healthy === 1 ? '' : 's'} · deletion stays disabled</p></div></aside></section><section class="metric-grid overview-metrics"><article class="metric-card"><p>Active productions</p><strong>${activeProductions || '—'}</strong><span>${activeProductions ? 'Workspaces in progress' : 'Add a production when work begins'}</span></article><article class="metric-card"><p>Connected sources</p><strong>${healthy}<em> / ${dashboard.integrations.length || 0}</em></strong><span>${healthy ? 'Available to Console' : 'Set up a source to begin'}</span></article><article class="metric-card"><p>Premiere workstations</p><strong>${pairedPremiereWorkstations || '—'}</strong><span>${pairedPremiereWorkstations ? 'Paired to production work' : 'No workstations paired yet'}</span></article></section><section class="split-grid overview-grid"><article class="panel timeline-panel"><div class="panel-heading"><div><p class="eyebrow">Today</p><h2>What changed</h2></div><a href="/console?view=activity">View all</a></div><div class="main-activity-feed">${activityRows}</div></article><article class="panel action-panel"><p class="eyebrow">Suggested next step</p><h2>${google && google.status === 'healthy' ? 'Bring a production into focus' : 'Connect the first source'}</h2><p>${google && google.status === 'healthy' ? 'Google Drive is available. Open a production workspace to organize the files and conversation around that work.' : 'Start with the systems your team already uses. Console asks only for the access needed by each feature.'}</p><div class="button-stack">${connectionActions || '<a class="button primary" href="/console?view=integrations">Set up integrations</a>'}</div></article></section>`;
  ...
  const activityView = `<section class="page-heading"><p class="eyebrow">Operational record</p><h1>Activity</h1><p>A normalized timeline of authorized source changes, created for production context rather than notification overload.</p></section><section class="panel full-panel"><div class="activity-page-feed">${activityRows}</div></section>`;
  ...
  const liveRefresh = !notice && (['overview', 'activity', 'slack'].includes(view) || (view === 'productions' && selectedProductionId))
...
function responsePage(body, headers = {}) {
  ...
  const visualPolishStyles = `<style>
    body{background:radial-gradient(circle at 84% -18%,rgba(81,116,255,.2),transparent 34rem),radial-gradient(circle at 22% 0,rgba(40,210,255,.06),transparent 28rem),#0a0e16}
    .app{grid-template-columns:232px minmax(0,1fr)}
    .rail{padding:20px 12px;background:rgba(12,17,27,.84);backdrop-filter:blur(24px);border-right-color:rgba(203,219,243,.1)}
    .brand{padding:7px 9px}.workspace{margin:31px 10px 12px}.nav{display:block}.nav-group{display:grid;gap:3px}.nav-label{margin:22px 10px 7px;color:var(--dim);font-size:.6rem;font-weight:800;letter-spacing:.12em;text-transform:uppercase}.system-nav .nav-item{padding-top:8px;padding-bottom:8px;font-size:.77rem;opacity:.74}.nav-item{border-radius:10px;transition:background .18s ease,color .18s ease}.nav-item.active{background:linear-gradient(90deg,rgba(70,124,255,.18),rgba(70,124,255,.04));box-shadow:inset 2px 0 var(--cyan)}
    .content{padding:30px 46px 72px}.topbar,main{max-width:1260px}.topbar{margin-bottom:30px}.hero{align-items:center;margin-bottom:25px}.hero h1{font-size:clamp(2.6rem,4.5vw,4.3rem)}.hero-copy{max-width:510px;margin-top:12px;font-size:.96rem}.hero-status{min-width:240px;padding:15px 17px;border-radius:16px;border-color:rgba(190,211,242,.13);background:rgba(20,28,43,.6);box-shadow:0 15px 38px rgba(0,0,0,.1)}
    .metric-grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.metric-card{min-height:120px;padding:18px 19px;border-radius:18px;border-color:rgba(190,211,242,.13);background:linear-gradient(145deg,rgba(28,38,57,.86),rgba(15,21,33,.9));box-shadow:0 16px 42px rgba(0,0,0,.14)}.metric-card strong{margin:7px 0 3px;font-size:1.7rem}.metric-card span{font-size:.72rem}.overview-grid{margin-top:14px;grid-template-columns:minmax(0,1.3fr) minmax(310px,.7fr)}
    .panel,.provider-grid article,.storage-hero{border-radius:20px;border-color:rgba(190,211,242,.13);background:linear-gradient(145deg,rgba(23,32,48,.88),rgba(13,19,30,.92));box-shadow:0 18px 50px rgba(0,0,0,.12)}.panel{padding:21px}.panel-heading{margin-bottom:15px}.panel-heading h2{font-size:1.14rem}.action-panel{display:flex;min-height:100%;flex-direction:column;justify-content:center}.action-panel p:not(.eyebrow){max-width:390px}.button-stack{margin-top:20px}.event{padding:13px 0}.event strong{font-size:.82rem}.event p{font-size:.75rem}.main-activity-feed,.production-activity-feed,.activity-page-feed,.production-slack-feed,.main-slack-feed{padding-right:8px}
    .page-heading{margin-bottom:25px}.page-heading h1{font-size:clamp(2.4rem,4vw,3.8rem)}.production-metrics{grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;margin-bottom:14px}.production-metrics article{min-height:104px;padding:17px 18px;border-radius:17px;border-color:rgba(190,211,242,.13);background:linear-gradient(145deg,rgba(28,38,57,.86),rgba(15,21,33,.9));box-shadow:0 14px 38px rgba(0,0,0,.11)}.production-metrics article:nth-child(4){display:none}.production-context-grid{gap:14px;margin-bottom:14px}.production-slack-compose,.premiere-presence-panel,.production-member-panel{margin-bottom:14px}.slack-panel,.slack-compose,.slack-alerts{border-radius:20px;border-color:rgba(190,211,242,.13);background:linear-gradient(145deg,rgba(23,32,48,.88),rgba(13,19,30,.92))}
    @media(max-width:1080px){.content{padding:26px 28px 56px}.app{grid-template-columns:214px minmax(0,1fr)}}
    @media(max-width:760px){.app{display:block}.rail{padding:15px}.nav{display:grid;grid-template-columns:repeat(3,1fr);gap:4px}.nav-group{display:contents}.nav-label,.system-nav{display:none}.content{padding:20px 14px 42px}.metric-grid,.production-metrics{grid-template-columns:1fr 1fr}.overview-grid{grid-template-columns:1fr}.hero-status{width:100%}}
    @media(max-width:420px){.metric-grid,.production-metrics{grid-template-columns:1fr}}
  </style>`;
  ...
  const pairingRecoveryStyles = '<style>html,body{overflow-x:hidden!important;background:#080b12}.app{width:100%;min-width:0;overflow-x:hidden}.notice{white-space:pre-wrap}.source-row-actions{display:grid;justify-items:end;gap:4px}.source-row-actions form,.pairing-actions form{margin:0}.pairing-actions{display:flex;gap:12px;align-items:center}.production-slack-feed,.main-slack-feed,.main-activity-feed,.production-activity-feed,.activity-page-feed{max-height:410px;overflow-y:auto;padding-right:10px;scrollbar-color:rgba(55,217,255,.55) transparent;scrollbar-width:thin}.activity-page-feed{max-height:680px}.production-slack-feed::-webkit-scrollbar,.main-slack-feed::-webkit-scrollbar,.main-activity-feed::-webkit-scrollbar,.production-activity-feed::-webkit-scrollbar,.activity-page-feed::-webkit-scrollbar{width:7px}.production-slack-feed::-webkit-scrollbar-thumb,.main-slack-feed::-webkit-scrollbar-thumb,.main-activity-feed::-webkit-scrollbar-thumb,.production-activity-feed::-webkit-scrollbar-thumb,.activity-page-feed::-webkit-scrollbar-thumb{border-radius:99px;background:rgba(55,217,255,.5)}</style>';
  const brandedBody = String(body)
    ...
    .replace('</head>', `${driveStyles}${driveControlStyles}${slackStyles}${workspaceStyles}${productionStyles}${pairingRecoveryStyles}${visualPolishStyles}</head>`);
