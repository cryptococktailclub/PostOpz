...
async function dashboardData(config, accessToken) {
  try {
    ...
    return { organizations: [], productions: [], integrations: [], activity: [], recommendations: [], memberships: [], workspaceFiles: [], productionLinks: [], sourceEvents: [] };
  }
}
...
function workspaceFilesPanel(dashboard, requestToken, selectedProductionId = '', productionOnly = false) {
...
function productionWorkspaceView(dashboard, requestToken, productionId) {
  const production = (dashboard.productions || []).find((item) => item.id === productionId);
...
function consolePage(user, dashboard, notice = '', requestToken = '', selectedView = 'overview', googleSession = { state: 'disconnected' }, slackSession = { state: 'unregistered' }, selectedProductionId = '') {
  ...
  const productionView = selectedProductionId && dashboard.productions.some((item) => item.id === selectedProductionId)
    ? productionWorkspaceView(dashboard, requestToken, selectedProductionId)
    : productionDirectory;
  ...
  const liveRefresh = ['overview', 'activity', 'slack'].includes(view) ? '<meta http-equiv="refresh" content="10">' : '';
...
function responsePage(body, headers = {}) {
  ...
  const brandedBody = String(body)
    ...
    .replace('</head>', `${driveStyles}${driveControlStyles}${slackStyles}${workspaceStyles}${productionStyles}</head>`);
    ...
    if (result.ok) {
      ...
      const message = form.action === 'register_production'
        ...
        : form.action === 'register_integration'
          ...
          : result.message;
      return responsePage(consolePage(user, dashboard, message, requestToken, selectedView, googleDrive, slackSession, selectedProductionId));
    }
    return responsePage(consolePage(user, dashboard, result.message, requestToken, selectedView, googleDrive, slackSession, selectedProductionId));
  }

  return responsePage(consolePage(user, dashboard, queryNotice, requestToken, selectedView, googleDrive, slackSession, selectedProductionId));
};
