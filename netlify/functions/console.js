...
async function dashboardData(config, accessToken) {
  try {
    const requests = await Promise.all([
      ...
      supabaseRequest(config, '/rest/v1/integration_connections?select=id,provider,display_name,status,last_synced_at,configuration&order=provider', { accessToken }),
...
async function slackBrowser(dashboard, query = {}) {
  ...
  try {
    ...
    return { state: 'ready', channels, selected, messages, search, threadTs, teamId: configuration.team_id || '' };
  } catch (_) {
    return { state: 'error', channels, selected, search, threadTs, message: 'Console could not reach Slack right now.' };
  }
}

function slackPage(slack) {
  const intro = '<section class="page-heading"><p class="eyebrow">Slack · Read only</p><h1>Team activity</h1><p>Browse selected public Slack channels inside Console. Message text is fetched live for this screen and is not copied into the Console database.</p></section>';
  ...
  return `${intro}${control}<div class="drive-toolbar slack-toolbar">${back}<a href="/console/slack/connect">Refresh channel index</a></div><section class="slack-panel"><div class="panel-heading"><div><p class="eyebrow">${slack.threadTs ? 'Conversation thread' : 'Live channel view'}</p><h2>#${escapeHtml(slack.selected.name)}</h2></div><span class="status-pill healthy"><i></i> Read only</span></div>${rows}</section>`;
}
...
function consolePage(user, dashboard, notice = '', requestToken = '', selectedView = 'overview', googleSession = { state: 'disconnected' }, slackSession = { state: 'unregistered' }) {
  ...
  const connectionActions = `${google ? `<a class="button primary" href="/console/google/connect">${google.status === 'healthy' ? 'Refresh Google Drive' : 'Connect Google Drive'}</a>` : ''}${slack ? `<a class="button secondary" href="/console/slack/connect">${slack.status === 'healthy' ? 'Refresh Slack' : 'Connect Slack'}</a>` : ''}${frame ? `<a class="button secondary" href="/console/frameio/connect">Connect Frame.io</a>` : ''}`;
  ...
  const pages = { overview, productions: productionView, media: googleMediaPage(googleSession, dashboard, requestToken), slack: slackPage(slackSession), integrations: integrationView, storage: storageView, activity: activityView, settings: settingsView };
...
function responsePage(body, headers = {}) {
  ...
  const slackStyles = '<style>.slack-controls{display:grid;grid-template-columns:minmax(180px,.48fr) minmax(200px,1fr) auto;gap:9px;align-items:end;margin:0 0 13px;padding:13px;border:1px solid var(--line);border-radius:12px;background:rgba(7,12,20,.5)}.slack-controls label{display:grid;gap:4px;color:var(--muted);font-size:.68rem;font-weight:750}.slack-controls input,.slack-controls select{width:100%;min-height:35px;border:1px solid var(--line);border-radius:7px;padding:7px 8px;background:#090e17;color:var(--text);font:inherit;font-size:.76rem}.slack-controls .button{min-height:35px}.slack-toolbar{margin-top:17px}.slack-panel{padding:23px;border:1px solid var(--line);border-radius:14px;background:linear-gradient(180deg,rgba(20,28,42,.88),rgba(12,17,26,.92))}.slack-message{display:grid;grid-template-columns:30px minmax(0,1fr);gap:10px;padding:15px 0;border-top:1px solid var(--line)}.slack-message:first-of-type{padding-top:0;border-top:0}.slack-avatar{display:grid;place-items:center;width:28px;height:28px;border-radius:8px;background:linear-gradient(135deg,#4a7fff,#8a53ff);font-size:.72rem;font-weight:850}.slack-message strong{font-size:.82rem}.slack-message time{margin-left:8px;color:var(--dim);font-size:.69rem}.slack-message p{max-width:780px;margin:5px 0 7px;color:#d7dfec;font-size:.8rem;line-height:1.6}.slack-actions{display:flex;gap:13px}.slack-actions a{color:var(--cyan);font-size:.71rem;font-weight:750}@media(max-width:760px){.slack-controls{grid-template-columns:1fr}.slack-controls .button{width:100%}}</style>';
