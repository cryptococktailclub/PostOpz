...
function consolePage(user, dashboard, notice = '', requestToken = '', selectedView = 'overview', googleSession = { state: 'disconnected' }) {
  ...
  const workspaceOptions = operatorWorkspaces.map((organization) => `<option value="${escapeHtml(organization.id)}">${escapeHtml(organization.name)}</option>`).join('');
  const nav = [
    ['overview', 'Overview', '◌'], ['productions', 'Productions', '◇'], ['media', 'Workspace Files', '▤'], ['integrations', 'Integrations', '⌁'],
  ...
  const pages = { overview, productions: productionView, media: googleMediaPage(googleSession, dashboard, requestToken), integrations: integrationView, storage: storageView, activity: activityView, settings: settingsView };
...
function responsePage(body, headers = {}) {
  ...
  const brandedBody = String(body)
    ...
    .replace('</head>', `${driveStyles}${driveControlStyles}${workspaceStyles}</head>`);
...
exports.handler = async (event) => {
  ...
  return responsePage(consolePage(user, dashboard, queryNotice, requestToken, selectedView, googleDrive));
};
