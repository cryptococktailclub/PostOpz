...
async function postSlackMessage(config, dashboard, user, form) {
  ...
  try {
    ...
    const source = await supabaseServiceRequest(config, '/rest/v1/source_events?on_conflict=organization_id,provider,provider_event_id', { method: 'POST', headers: { 'Content-Type': 'application/json', Prefer: 'resolution=ignore-duplicates,return=representation' }, body: JSON.stringify({ organization_id: connection.organization_id, integration_connection_id: connection.id, provider: 'slack', provider_event_id: providerEventId, occurred_at: new Date(Number(result.ts) * 1000).toISOString(), payload: { direction: 'outgoing', channel_id: channelId, thread_ts: threadTs || null, sender_id: user.id, text_sha256: digest }, payload_sha256: digest }) });
    const inserted = source.ok && Array.isArray(source.data) ? source.data[0] : null;
    if (inserted) await supabaseServiceRequest(config, '/rest/v1/activity_items', { method: 'POST', headers: { 'Content-Type': 'application/json', Prefer: 'return=minimal' }, body: JSON.stringify({ organization_id: connection.organization_id, source_event_id: inserted.id, kind: 'message_sent', title: `Console posted to Slack`, detail: 'A Console Operator sent a message to an approved Slack channel.', severity: 'info', occurred_at: inserted.occurred_at }) });
...
function slackPage(slack, requestToken) {
  ...
  return `${intro}${control}<div class="drive-toolbar slack-toolbar">${back}<a href="/console/slack/connect">Refresh channel index</a></div>${composer}<section class="slack-panel"><div class="panel-heading"><div><p class="eyebrow">${slack.threadTs ? 'Conversation thread' : 'Live channel view'}</p><h2>#${escapeHtml(slack.selected.name)}</h2></div><span class="status-pill healthy"><i></i> Read only</span></div>${rows}</section>${alerts}`;
}
