...
exports.handler = async (event) => {
  ...
  if (mode === 'select' && event.httpMethod === 'POST') {
    ...
    const summary = index ? `${index.observedMessageCount} Slack messages were refreshed.` : 'Slack channels connected. The first refresh will run shortly.';
    return response(303, '', { Location: `/console?view=slack&connected=${encodeURIComponent(summary)}`, 'Set-Cookie': `${shared.clearCookie(TOKEN_COOKIE)}, ${shared.clearCookie(STATE_COOKIE)}` });
  }
